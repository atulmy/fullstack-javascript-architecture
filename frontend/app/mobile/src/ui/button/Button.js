...
// UI Imports
import { buttonPadding, blockMarginHalf, buttonRadius, font } from '../common/responsive'
import {
  black, grey, grey1, grey2,
  highlight,
  primary,
  primaryLight,
  secondary,
...
const ButtonState = ({ disabled, onPress, theme, shadow, condensed, fullWidth, style = {}, children }) => (
  disabled
    ? <View
        style={[
          styles.container,
...
// Component
const Button = (props) => {
  const { title, onPress, iconLeft, iconRight, iconPack, theme, shadow, condensed, fullWidth, on, disabled, style = {} } = props
  ...
  return(
    ...
    >
      ...
      <Text style={[styles.text, { color: themes[theme].text, fontSize: condensed ? font(12) : font(16) }]}>{ title.toUpperCase() }</Text>
      ...
      { on &&
        <View style={{ position: 'absolute', top: 5, right: 5, height: 5, width: 5, borderRadius: 3, backgroundColor: highlight }} />
      }
    </ButtonState>
  )
}
